...
from .utils import scale_back, merge, save_concat_images, compile_frames_to_gif
...
class UNet(object):
    # embedding_num을 40 -> 11로 변경 (infer.py 과정에서의 에러)
    ...
    def validate_model(self, val_iter, epoch, step):
        ...
        print("=========================[Ratatou2mergency]========================")
        print(merged_pair)
        print("===================================================================")
        
        merged_pair = ((merged_pair + 1) / 2 * 255).astype(np.uint8)
        imsave(sample_img_path, merged_pair)
        
        
    def make_output(self, val_iter, save_dir, count, end):
        labels, images = next(val_iter)
        fake_imgs, real_imgs, d_loss, g_loss, l1_loss = self.generate_fake_samples(images, labels)
        print("[%05d]Output: d_loss: %.5f, g_loss: %.5f, l1_loss: %.5f" % (count, d_loss, g_loss, l1_loss))
        ...
        for idx in range(self.batch_size):
            if count+idx >= end: 
                break
            sample_img_path = os.path.join(save_dir, "%05d.jpg" % (count+idx))
            imsave(sample_img_path, merged_pair[idx*256:(idx+1)*256, 256:, :])

#         sample_img_path = os.path.join(save_dir, "output_%05d.png" % (count))
#         imsave(sample_img_path, merged_pair)
    ...
    # 추론 과정을 위한 train 편집 함수
    def infer_by_train(self, lr=0.0002, epoch=100, schedule=10, resume=True, flip_labels=False,
              freeze_encoder=False, fine_tune=None, sample_steps=50, checkpoint_steps=500, source_obj='', save_dir='', embedding_ids=1):
        ...
        learning_rate = tf.placeholder(tf.float32, name="learning_rate")  # 학습률
        ...
        # filter by one type of labels / 데이터 공급자 세팅 
...
#         val_batch_iter = data_provider.get_val_iter(self.batch_size)
        inject_provider = InjectDataProvider(source_obj)
#         val_batch_iter = inject_provider.get_single_embedding_iter_all(self.batch_size, embedding_ids)
        val_batch_iter = inject_provider.get_single_embedding_iter(self.batch_size, embedding_ids)
        ...
        time = math.ceil(len(inject_provider.data.examples) / self.batch_size)
        end = len(inject_provider.data.examples)
        for i in range(0, time):
            self.make_output(val_batch_iter, save_dir, i*32, end)
...
#                     self.make_output(val_batch_iter, save_dir, ei, counter)                
    ...
    def infer(self, source_obj, embedding_ids, model_dir, save_dir):
        ...
        train_provider = TrainDataProvider('experiment/data')
        source_iter = train_provider.get_val_iter(self.batch_size)
...
#             source_iter = source_provider.get_random_embedding_iter(self.batch_size, embedding_ids)
        ...
        for labels, source_imgs in source_iter:
            ...
            merged_pair = np.concatenate([merged_real_images, merged_fake_images], axis=1)
            merged_pair = ((merged_pair + 1) / 2 * 255).astype(np.uint8)
            imsave(f'{save_dir}/checking{labels}.png', merged_pair)
