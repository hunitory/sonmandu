...
def pickle_examples(paths, train_path, val_path, label, train_val_split=0.1):
    """
...
parser.add_argument('--label', type=int, dest='label',
                    help='label of train and val')
args = parser.parse_args()

if __name__ == "__main__":
    train_path = os.path.join(args.save_dir, "train.obj")
    val_path = os.path.join(args.save_dir, "val.obj")
    pickle_examples(sorted(glob.glob(os.path.join(args.dir, "*.jpg"))), train_path=train_path, val_path=val_path, label=args.label, train_val_split=args.split_ratio)
